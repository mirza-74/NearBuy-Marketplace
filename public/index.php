...
// filter hanya kebutuhan sehari hari
...
// Jika tidak ada lokasi user atau tidak ada hasil, fallback produk umum
if (empty($rekom)) {
    $sqlFallback = "
        SELECT 
            p.id,
  ...
  <div class="content">
    ...
    <section class="search">
      <form method="get" action="<?= e($BASE) ?>/index.php">
        <input type="text" name="q" value="<?= e($searchQ) ?>" maxlength="80"
    ...
    <section class="produk-section">
      ...
      <?php if ($userLat === null || $userLng === null): ?>
        <p style="font-size:0.9rem;color:#64748b;margin-bottom:10px;">
          Aktifkan lokasi di halaman pengaturan NearBuy agar rekomendasi benar benar sesuai lokasi kamu.
        </p>
      <?php endif; ?>
    ...
    <section class="produk-section">
      <h2>
        ...
        <?php if (!empty($searchQ)): ?> untuk “<?= e($searchQ) ?>”<?php endif; ?>
      </h2>
      <div class="produk-grid">
        <?php if (!empty($produk)): ?>
      ...
      <?php if ($totalPages > 1): ?>
        <nav class="pagination" style="margin-top:12px; display:flex; gap:10px; align-items:center;">
          ...
          <?php if ($page > 1): $qs['page'] = $page - 1; ?>
            <a href="<?= e($BASE) ?>/index.php?<?= http_build_query($qs) ?>"
               style="padding:8px 12px; border:1px solid #e5e7eb; border-radius:8px; text-decoration:none;">« Prev</a>
          <?php endif; ?>
          <span>Halaman <?= (int)$page ?> dari <?= (int)$totalPages ?></span>
          <?php if ($page < $totalPages): $qs['page'] = $page + 1; ?>
            <a href="<?= e($BASE) ?>/index.php?<?= http_build_query($qs) ?>"
               style="padding:8px 12px; border:1px solid #e5e7eb; border-radius:8px; text-decoration:none;">Next »</a>
          <?php endif; ?>
        </nav>
      <?php endif; ?>
    </section>

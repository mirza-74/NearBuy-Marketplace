...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // ====== SETUJUI PEMBAYARAN LANGGANAN (ACC) - PERBAIKAN DILAKUKAN DI SINI ======
  if ($action === 'approve_payment') {
    try {
      ...
      // Anda harus memastikan nama paket ini konsisten dengan yang Anda gunakan di `pilih_paket.php`
      if (strcasecmp($packageCode, 'Paket Starter') === 0 || strcasecmp($packageCode, 'Paket A') === 0) {
        $newProductLimit = 15; // Contoh: Paket Starter/A = 15 Produk
      } elseif (strcasecmp($packageCode, 'Paket Premium') === 0 || strcasecmp($packageCode, 'Paket B') === 0) {
        $newProductLimit = 99999; // Contoh: Paket Premium/B = Tak Terbatas
      }
            // Jika ada paket lain, tambahkan di sini

      // 3. Update status langganan dan product_limit
      $stmt = $pdo->prepare("
        UPDATE shops SET 
          subscription_status = 'active', 
          product_limit = ?, /* BARIS PENTING: MENYETEL BATAS PRODUK */
          updated_at = NOW() 
        WHERE id = ? LIMIT 1
      ");
      $stmt->execute([$newProductLimit, $shopId]);

      $_SESSION['flash_admin'] = "Pembayaran Toko ID {$shopId} berhasil disetujui. Langganan **{$packageCode}** diaktifkan dengan batas **{$newProductLimit}** produk.";
    } catch (Throwable $e) {
    ...
    try {
      ...
      $stmt = $pdo->prepare("
        UPDATE shops SET 
          ...
          product_limit = 0, /* RESET BATAS PRODUK KE 0 */

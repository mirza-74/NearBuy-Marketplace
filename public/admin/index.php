...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reject_shop') {
        try {
            // gunakan is_active = 2 untuk status "ditolak"
    ...
    if ($action === 'approve_payment') {
        try {
            ...
            if (!$shopData || empty($shopData['package_code'])) {
                $_SESSION['flash_admin'] = "Gagal menyetujui pembayaran: Data paket untuk Toko ID {$shopId} tidak ditemukan.";
                header("Location: {$BASE}/admin/index.php");
                exit;
            }
            ...
            // 2. Tentukan batas produk berdasarkan nama paket (SAMAKAN DENGAN paket.php)
            if (strcasecmp($packageCode, 'Paket Starter') === 0) {
                $newProductLimit = 15;
            } elseif (strcasecmp($packageCode, 'Paket Premium') === 0) {
                $newProductLimit = 9999; // tanpabatas (secara logika)
            }

            // 3. Update status langganan & batas produk
            $stmt = $pdo->prepare("
                UPDATE shops SET 
                    ...
                    package_expires_at  = DATE_ADD(NOW(), INTERVAL 30 DAY),
                    updated_at          = NOW()
                WHERE id = ? 
                LIMIT 1
            ");
            $stmt->execute([$newProductLimit, $shopId]);

            $_SESSION['flash_admin'] = "Pembayaran Toko ID {$shopId} berhasil disetujui. Paket {$packageCode} aktif dengan batas {$newProductLimit} produk.";
        } catch (Throwable $e) {
    ...
    if ($action === 'reject_payment') {
        try {
            ...
            $_SESSION['flash_admin'] = "Pembayaran Toko ID {$shopId} ditolak. Status dikembalikan ke 'none'.";
        } catch (Throwable $e) {
...
try {
    $stmt = $pdo->prepare("
        SELECT 
            s.id, s.name, s.address, s.is_active, s.created_at, 
            u.full_name, u.email,
...
try {
    $stmt = $pdo->prepare("
        SELECT 
            s.id, s.user_id, s.name, s.address, s.latitude, s.longitude, 
...
// ===============================================================
// VERIFIKASI PEMBAYARAN LANGGANAN (package_status = 'waiting_payment')
// ===============================================================
$pendingPayments = [];
try {
    $stmt = $pdo->prepare("
        ...
        FROM shops s
        JOIN users u ON u.id = s.user_id
        WHERE s.package_status = 'waiting_payment'
            ...
            <div class="shop-request-main">
              <div class="shop-name payment-info-title">
                ...
                <span style="color:#10b981;font-weight:700;">
                  <?= e($pay['package_code'] ?? 'TIDAK DIKETAHUI') ?>
                </span>
              </div>
        ...
        <?php foreach ($requests as $req): ?>
          <div class="shop-request-card">
            <div class="shop-request-main">
              ...
              <div class="shop-coord">
                Koordinat: <?= e((string)$req['latitude'] ?? '-') ?>, <?= e((string)$req['longitude'] ?? '-') ?>
                · Request: <?= e($req['created_at']) ?>
              </div>
            </div>
          ...
          <div class="note-card">
            ...
            <div class="note-amount">
              <div class="note-money">
                Rp<?= number_format((float)($n['grand_total'] ?? 0), 0, ',', '.') ?>
              </div>
